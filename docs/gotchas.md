...
| 9 | :warning: | [TrustyAI / NeMo](#trustyai--safety-team) | [No built-in auth — can't verify caller identity or exchange tokens](gotchas-nemo-guardrails.md#gotcha-10-no-built-in-authentication) | Needs auth proxy sidecar if Llama Stack uses JWT auth |
